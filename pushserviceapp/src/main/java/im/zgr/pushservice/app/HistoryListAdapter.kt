...
import im.zgr.pushservice.rest.ApiServerImpl
import java.util.*

class HistoryListAdapter : RecyclerView.Adapter<HistoryListAdapter.ViewHolder>() {
    var values: MutableList<NotificationDto> = mutableListOf()
    var onDeleteItem: ((notificationDto: NotificationDto, index: Int) -> Unit)? = null
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        notificationParts.add("status: " + when(item.status) {
            ApiServerImpl.ContentStatus.New.toString() -> "новое"
            ApiServerImpl.ContentStatus.Seen.toString() -> "просмотрено"
            ApiServerImpl.ContentStatus.Deleted.toString() -> "удалено"
            else -> item.status
        })
        ...
        if(category == NotificationDto.IMAGE) {
        ...
        setOnClick(holder.itemView, item)

    }

    private fun setOnClick(view: View, item: NotificationDto) {
        view.setOnClickListener {
            val app = view.context.applicationContext as AppBasic
            if (item.contentUrl != null) {
                val category = item.contentCategory?.toLowerCase(Locale.getDefault())
                val url = item.contentUrl.toString()
                app.startView(item.title, item.text, category, url)
            }
...
//            mAdapter.onItemMove(viewHolder.adapterPosition, target.adapterPosition)
        ...
        override fun onSwiped(
            ...
            adapter.onDeleteItem?.invoke(adapter.values[pos], pos)
        }
